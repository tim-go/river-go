...
if [[ "$ENV" != "staging" && "$ENV" != "prod" ]]; then
  fail "Usage: seed-river-geometry.sh <staging|prod>"
  print_summary "river-geometry seed"
  exit 1
fi
...
section "River-geometry seed preflight - $ENV"
ok=true
...
section "Run river-geometry seed"
DATABASE_URL="$DATABASE_URL" \
  npm --prefix "$REPO_DIR/api" run seed:river-geometry

pass "river-geometry seed completed"
print_summary "river-geometry seed"
