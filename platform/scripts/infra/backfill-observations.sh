...
while [[ "$#" -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      print_summary "observation backfill"
      exit 1
      ;;
  esac
  shift
...
if [[ "$ENV" != "staging" && "$ENV" != "prod" ]]; then
  fail "Usage: backfill-observations.sh <staging|prod> [--hours <n>]"
  print_summary "observation backfill"
  exit 1
fi
...
section "Observation backfill preflight - $ENV"
ok=true
...
section "Run observation backfill"
DATABASE_URL="$DATABASE_URL" \
  npm --prefix "$REPO_DIR/api" run backfill:observations -- "--hours=$HOURS"

pass "observation backfill completed"
print_summary "observation backfill"
