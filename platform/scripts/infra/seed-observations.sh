...
if [[ "$ENV" != "staging" && "$ENV" != "prod" ]]; then
  fail "Usage: seed-observations.sh <staging|prod>"
  print_summary "observation seed"
  exit 1
fi
...
section "Observation seed preflight - $ENV"
ok=true
...
section "Run observation seed"
DATABASE_URL="$DATABASE_URL" \
  npm --prefix "$REPO_DIR/api" run seed:observations

pass "observation seed completed"
print_summary "observation seed"
