...
The first public photo implementation may replace or extend this contribution-scoped table with a more general photo metadata table if section-level and POI-level photos need to exist without a parent contribution. The durable product workflow is defined in `/docs/specs/contributions/photo-uploads.md`.
...
defined in `/docs/specs/foundations/geospatial-domain-model.md`.
...
Observation provider tables are defined in `/docs/specs/foundations/observation-ingestion.md`.
