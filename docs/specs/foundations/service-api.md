...
The backend should preserve the current prototype's section-detail and contribution workflows while supporting the river-first discovery direction. Identity, persistence, moderation, media, provider ingestion, and river/section relationships should move out of the browser.
...
| `GET` | `/api/route-suggestions/approved` | Public reviewed route suggestions for low-confidence candidate section display. |
...
| `GET` | `/api/rivers` | Public read-only canonical river summary list for river-first discovery. |
| `GET` | `/api/rivers/:riverId` | Public read-only canonical river detail with section links and candidate counts. |
| `GET` | `/api/rivers/:riverId/map-pois` | Public confirmed/promoted map POIs linked to sections under a canonical river. |
...
| `GET` | `/api/moderation/source-candidate-pois?riverId=...&status=...&limit=...` | Admin/moderator source-derived candidate POI queue. |
| `POST` | `/api/moderation/source-candidate-pois/:id/status` | Admin/moderator source-candidate status update for `review_needed`, `confirmed`, `rejected`, or `merged`. |
...
| API-F14 | Photo management API | Backend/media | Active | MVP | — | Supports member photo listing and owner/moderator soft-delete. |
| API-F15 | Member contribution management API | Backend/community | Active | MVP | — | Supports member contribution listing and owner/moderator soft-delete. |
| API-F16 | what3words location API | Backend/integration | Active | MVP | — | Server-side coordinate/address conversion with optional sync-time contribution enrichment. |
| API-F17 | Profile public name and ICE API | Backend/profile | Active | MVP | — | Supports member public-name updates and owner-only emergency-contact profile read/write. |
| API-F18 | Public candidate section API | Backend/API | Active | MVP | — | Exposes reviewed route suggestions for public low-confidence candidate section display. |
| API-F19 | Watercourse snap API | Backend/geospatial | Active | MVP | — | Uses stored OSM waterways to snap route control points and route along connected river geometry with confidence/warnings. |
| API-F20 | Watercourse viewport API | Backend/geospatial | Active | MVP | — | Exposes clipped, simplified OSM waterway reference lines for the `Known rivers` map overlay. |
| API-F21 | OSM waterway import | Backend/data | Active | MVP | — | Imports OSM waterway geometry into PostGIS as the visual route-snapping layer. |
| API-F22 | Canonical river read API | Backend/API | Active | MVP | — | Exposes public canonical river summaries and details for river-first discovery. |
| API-F23 | Source candidate moderation API | Backend/admin | Active | MVP | — | Exposes moderator list/update endpoints for source-derived candidate POI review. |
| API-F24 | River promoted POI read API | Backend/API | Active | MVP | — | Exposes confirmed/promoted source-backed map POIs for the selected canonical river. |
...
| 2026-05-25 | Added public reviewed route suggestion read endpoint. |
| 2026-06-05 | Aligned backend purpose with river-first discovery and fixed duplicate feature keys. |
