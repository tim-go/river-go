...
| River-first discovery | `/docs/specs/discovery/river-first-discovery.md` | Active | Built: canonical river markers/list, selected-river context, level-band colouring, discipline filter, and the **river detail page** (F3, first cut 2026-07-04 — dedicated `/river/<id>` page: hero, overview, sections, grouped POIs with map-jump, photos, levels chart, locator map, data gaps). Queued: **nearby-river list** (F4), bigger levels-history chart, candidate sections, recent reports, SSR/SEO (B8). |
...
| Map entity surfaces (framework) | `/docs/specs/map-entity-surfaces.md` | **Agreed shape (design)** | One-entity / two-surfaces (panel · page) / capability-driven sections, on top of the `pois` index. Unblocks **first-class amenities** (notes/photos on car parks + campsites via generic `poi_id`) and pays down surface duplication (popup-only amenities, dual contribution key, ~3× marker/click, bespoke river page). Amenities-first delivery. River attribution kept separate. |
...
| Group membership & invites v2 | `/docs/specs/group-tools/group-membership-and-invites.md` | **Landed** | `/api/members/search` retired; invite-by-exact-email, group link + request-to-join, access modes (request_to_join / invite_only), invite/request management, ownership transfer, role promote/demote, membership audit (migration 032). **Directory listing decoupled from page visibility** (new `groups.listing` visible/hidden default visible — a private club can be listed and still gate its page; migration 042). Parked: non-member email invites (F7), public/discoverable groups (F8). |
